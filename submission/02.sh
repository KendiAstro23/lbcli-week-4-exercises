# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""
# 1. Get the TXID of the provided transaction
TXID=$(btc -regtest decoderawtransaction "$transaction" | jq -r '.txid')

# 2. Creating the timelocked raw transaction
RAW_TX=$(btc -regtest createrawtransaction \
  '[
    {"txid":"'"$TXID"'", "vout":0, "sequence":2016},
    {"txid":"'"$TXID"'", "vout":1, "sequence":2016}
  ]' \
  '{
    "2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP": 0.20000000,
    "bcrt1qg09ftw43jvlhj4wlwwhkxccjzmda3kdm4y83ht": 0.03600000
  }' \
  0 true)

echo "Raw Timelocked Transaction:"
echo "$RAW_TX"